...
class BacktestEngine(DILadderExecutionMixin):
    def __init__(self, data: pd.DataFrame, config: BacktestConfig, intrabar_data: pd.DataFrame | None = None, progress_callback: Callable[[int, int, int, int], None] | None = None, progress_interval: int = 50):
        ...
        self.sr_detector = SupportResistanceDetector(
            ...
            zone_padding_atr=getattr(config, "sr_zone_padding_atr", 0.10),
            max_cluster_span_atr=getattr(config, "sr_zone_max_cluster_span_atr", 0.50),
            min_rejection_atr=getattr(config, "sr_min_rejection_atr", 0.50),
    ...
    def _begin_strategy_rule_trace(self,i,regime,direction,profile_key_value,profile):
        ...
        for number,rule in enumerate(getattr(profile,"entry_rules",())):
            if "_builder_id" not in rule:
                continue
            kind=str(rule.get("_builder_kind","")).upper()

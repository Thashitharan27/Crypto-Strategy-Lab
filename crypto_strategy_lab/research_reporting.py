...
def _write_optional_diagnostics(
    ...
    """Generate only explicitly requested human/diagnostic outputs.

    The canonical trade/context artifacts are already complete before this runs.
    Diagnostics operate on copies or on passive telemetry and never mutate the
    authoritative trades used for fingerprinting or strategy outcomes.
    """
    requested = any(
        (
...
class CsvManifestReporter:
    """Publish mandatory artifacts, optional diagnostics, then the manifest."""
    ...
    def report(self, result, context):
        ...
        # Task 16 owns the one authoritative trade/context Parquets. These are
        # never optional because completed runs must remain reproducible/queryable.
        ...
        workbook = None
        if context.config.reporting.create_human_workbook:
        ...
        if workbook is not None:
            artifacts["workbook"] = _catalog_entry(workbook, run_dir, "xlsx", None)

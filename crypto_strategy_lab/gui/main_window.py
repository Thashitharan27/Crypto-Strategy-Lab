...
class MainWindow(QMainWindow):
    ...
    def _build_config(self):
        ...
        self.account_risk_help=QLabel("The sizing budget controls the dollars used to calculate quantity. By default it equals planned full-stop exposure; a separate position-sizing stop can intentionally decouple those values."); self.account_risk_help.setWordWrap(True)
        for lab,w in [("Starting Equity",self.equity),("Base Sizing Budget Per Trade",self.risk_leg),("Effective Sizing Budget",self.risk_warn),("Maximum Leverage Per Trade",self.max_lev_leg),("Maximum Portfolio Leverage",self.max_lev_combined),("",self.account_risk_help)]: risk.addRow(lab,w)
    ...
    def _build_support_resistance_tab(self):
        ...
        af=QFormLayout(); self.sr_zone_width_atr.setSuffix(" ATR"); self.sr_zone_width_atr.setToolTip("Merge validated rejection zones only when the gap between their edges is within this ATR distance.")
        ...
        self._sr_detection_presets={"Conservative":{"pivot_left":8,"pivot_right":8,"lookback":300,"zone_width_atr":0.25,"break_tolerance_atr":0.35},"Balanced (Recommended)":{"pivot_left":5,"pivot_right":5,"lookback":200,"zone_width_atr":0.15,"break_tolerance_atr":0.25},"Sensitive":{"pivot_left":3,"pivot_right":3,"lookback":150,"zone_width_atr":0.10,"break_tolerance_atr":0.15}}

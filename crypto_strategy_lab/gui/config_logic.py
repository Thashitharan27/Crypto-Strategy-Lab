...
DEFAULT_GUI_CONFIG: dict[str, Any] = {
    ...
    "enable_di_regime_reward_risk": False, "bear_regime_return_threshold": -0.20,
...
def validate_config_values(values: dict[str, Any], require_paths: bool = True) -> list[str]:
    ...
    try:
        bear_threshold = float(values.get("bear_regime_return_threshold", -1))
...
_OBSOLETE_EXACT = {
    ...
    "enable_coin_flip_sizing","coin_flip_seed","coin_flip_large_multiplier","coin_flip_small_multiplier","enable_di_direction_sizing","di_execution_mode","flip_filtered_di_direction",
...
def canonical_config_values(values: dict[str, Any]) -> dict[str, Any]:
    ...
    always_keep = {
        "enable_di_direction_selection", "enable_di_pressure_analysis", "di_pressure_lookback",
        "bear_regime_return_threshold", "bull_regime_lookback_days", "bull_regime_return_threshold",
    }
    result={}
    for key,value in values.items():
        if key not in DEFAULT_GUI_CONFIG:
            continue
        if key in always_keep or key not in _OBSOLETE_EXACT and not key.startswith(_OBSOLETE_PREFIXES): result[key]=value

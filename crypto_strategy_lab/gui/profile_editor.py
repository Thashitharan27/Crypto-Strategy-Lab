...
class StrategyProfilesWidget(QWidget):
    ...
    def values(self): return {"enable_strategy_profiles":True,"strategy_profile_run_mode":self.mode.currentData(),"market_regime_method":self.regime_method.currentData(),"structural_regime_sma_days":self.structural_sma_days.value(),"structural_regime_slope_lookback_days":self.structural_slope_days.value(),"bull_regime_lookback_days":self.regime_lookback.value(),"bull_regime_return_threshold":self.bull_threshold.value()/100.0,"bear_regime_return_threshold":self.bear_threshold.value()/100.0,"adx_period":self.adx_period.value(),"bb_period":self.bb_period.value(),"bb_stddevs":self.bb_stddevs.value(),"strategy_profiles":profiles_to_dict(self.profiles)}
    def apply_values(self,values):
        self.profiles=normalize_profiles(values.get("strategy_profiles",{})); index=self.mode.findData(str(values.get("strategy_profile_run_mode","COMBINED_SHARED_CAPITAL"))); self.mode.setCurrentIndex(max(0,index)); method=self.regime_method.findData(str(values.get("market_regime_method","ASSET_RETURN"))); self.regime_method.setCurrentIndex(max(0,method)); self.structural_sma_days.setValue(int(values.get("structural_regime_sma_days",200))); self.structural_slope_days.setValue(int(values.get("structural_regime_slope_lookback_days",30))); self.regime_lookback.setValue(int(values.get("bull_regime_lookback_days",90))); self.bull_threshold.setValue(float(values.get("bull_regime_return_threshold",.20))*100); self.bear_threshold.setValue(float(values.get("bear_regime_return_threshold",-.20))*100); self.adx_period.setValue(int(values.get("adx_period",14))); self.bb_period.setValue(int(values.get("bb_period",20))); self.bb_stddevs.setValue(float(values.get("bb_stddevs",2))); self._update_regime_controls(); self._load(); self._refresh_list()

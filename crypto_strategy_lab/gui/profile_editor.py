...
class StrategyProfilesWidget(QWidget):
    ...
    def __init__(self):
        ...
        for label,widget in (("Return lookback",self.regime_lookback),("Bull starts at",self.bull_threshold),("Bear starts at or below",self.bear_threshold),("ADX period",self.adx_period),("Bollinger period",self.bb_period),("Bollinger deviations",self.bb_stddevs)): definition_form.addRow(label,widget)
        ...
        buttons=QHBoxLayout(); copy_btn=QPushButton("Copy Profile"); paste_btn=QPushButton("Paste Profile"); reset_btn=QPushButton("Reset Profile"); buttons.addWidget(copy_btn); buttons.addWidget(paste_btn); buttons.addWidget(reset_btn); self.form.addRow(buttons); self.editor_layout.addStretch(); self.clipboard=None
        copy_btn.clicked.connect(lambda:setattr(self,"clipboard",deepcopy(self.profiles[self.current]))); paste_btn.clicked.connect(self._paste); reset_btn.clicked.connect(self._reset); self.list.currentRowChanged.connect(self._select); self.mode.currentTextChanged.connect(self.changed); self.mode.currentIndexChanged.connect(self._update_mode_help)
        for widget in (self.regime_lookback,self.bull_threshold,self.bear_threshold,self.adx_period,self.bb_period,self.bb_stddevs): widget.valueChanged.connect(self.changed)
        ...
        self._update_mode_help(); self._refresh_list(); self.list.setCurrentRow(0)

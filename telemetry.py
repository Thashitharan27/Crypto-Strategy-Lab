...
def _at_or_before(group: pd.DataFrame, minutes: float, col: str):
    ...
    return finite(eligible.iloc[-1].get(col))
...
def add_journey_columns(trades: pd.DataFrame, telemetry: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in trades.iterrows():
        ...
        for ind in INDICATORS:
            s = pd.to_numeric(g.get(ind, pd.Series(dtype=float)), errors="coerce")
            ...
            out[f"{ind}_change"] = exitv - entry if pd.notna(exitv) and pd.notna(entry) else np.nan
            out[f"{ind}_change_first_hour"] = firsth - entry if pd.notna(firsth) and pd.notna(entry) else np.nan
            out[f"{ind}_slope_per_hour"] = out[f"{ind}_change"] / (holding / 60) if holding > 0 and pd.notna(out[f"{ind}_change"]) else np.nan
            if ind in ("bb_width", "atr"):
                out[f"{ind}_change_pct"] = out[f"{ind}_change"] / entry * 100 if pd.notna(entry) and entry else np.nan
...
def _ensure_journey_columns(trades: pd.DataFrame) -> pd.DataFrame:
    ...
    for ind in INDICATORS:
        ...
        if ind in ("bb_width","atr"): trades[f"{ind}_change_pct"] = np.nan
        for label in MILESTONES.values(): trades[f"{ind}_{label}"] = np.nan
    return trades


def trade_journey_analysis(trades: pd.DataFrame) -> pd.DataFrame:
    cols = ["outcome","trade_count","average_holding_hours","median_holding_hours","average_adx_entry","average_adx_max","average_adx_change_first_hour","average_adx_slope_per_hour","average_di_spread_entry","average_di_spread_max","average_di_spread_change_first_hour","average_di_spread_slope_per_hour","average_bb_width_entry","average_bb_width_max","average_bb_width_change_first_hour","average_bb_width_change_pct","average_bb_width_slope_per_hour","average_atr_entry","average_atr_max","average_atr_change_first_hour","average_atr_change_pct","average_atr_slope_per_hour","average_net_pnl","total_net_pnl"]
    ...
    for label in order:
        g=t[t.outcome==label]
        rows.append({"outcome":label,"trade_count":len(g),"average_holding_hours":g.holding_hours.mean(),"median_holding_hours":g.holding_hours.median(),"average_net_pnl":g.pair_net_pnl.mean(),"total_net_pnl":g.pair_net_pnl.sum(), **{f"average_{c}":g[c].mean() for c in ["adx_entry","adx_max","adx_change_first_hour","adx_slope_per_hour","di_spread_entry","di_spread_max","di_spread_change_first_hour","di_spread_slope_per_hour","bb_width_entry","bb_width_max","bb_width_change_first_hour","bb_width_change_pct","bb_width_slope_per_hour","atr_entry","atr_max","atr_change_first_hour","atr_change_pct","atr_slope_per_hour"] if c in g}})
    return pd.DataFrame(rows, columns=cols)


def winner_loser_journey_analysis(trades: pd.DataFrame) -> pd.DataFrame:
    metrics={"adx_entry":"ADX at entry","adx_max":"ADX maximum","adx_change_first_hour":"ADX first-hour change","adx_change":"ADX total change","di_spread_entry":"DI Spread at entry","di_spread_max":"DI Spread maximum","di_spread_change_first_hour":"DI Spread first-hour change","di_spread_change":"DI Spread total change","bb_width_entry":"BB Width at entry","bb_width_max":"BB Width maximum","bb_width_change_first_hour":"BB Width first-hour change","bb_width_change_pct":"BB Width total percentage change","atr_entry":"ATR at entry","atr_max":"ATR maximum","atr_change_first_hour":"ATR first-hour change","atr_change_pct":"ATR total percentage change","holding_hours":"Holding time","pair_net_pnl":"Net PnL"}
    rows=[]
...
def double_sl_journey_analysis(trades: pd.DataFrame, telemetry: pd.DataFrame) -> pd.DataFrame:
    ...
    for _,r in ds.iterrows():
        ...
        for ind in INDICATORS:
            row[f"{ind}_entry"]=r.get(f"{ind}_entry",np.nan); row[f"{ind}_at_first_sl"]=finite(before.iloc[-1][ind]) if len(before) else np.nan; row[f"{ind}_max_before_first_sl"]=before[ind].max() if len(before) else np.nan; row[f"{ind}_max_after_first_sl"]=after[ind].max() if len(after) else np.nan; row[f"{ind}_change_first_hour"]=r.get(f"{ind}_change_first_hour",np.nan)
        rows.append(row)
    return pd.DataFrame(rows)

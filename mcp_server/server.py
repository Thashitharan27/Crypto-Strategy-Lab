...
from crypto_strategy_lab.run_manifest import (RunArtifactError, artifact_path,
...
class BacktestReports:
    ...
    def _metadata(self, run: Path, manifest: dict[str, Any]) -> dict[str, Any]:
        request, hashes = manifest["request"], manifest["hashes"]
        return {
            "folder_name": run.name,
            "run_id": manifest["run_id"], "run_started_at": manifest["run_started_at"],
            "run_completed_at": manifest["run_completed_at"], "symbol": request["symbol"],
            ...
            "strategy_hash": hashes["strategy_hash"], "execution_hash": hashes["execution_hash"],
    ...
    def list_runs(self, limit: int = 50) -> list[dict[str, Any]]:
        ...
        for path in self.root.iterdir():
            ...
            try: manifest = load_completed_manifest(path)
            except RunArtifactError: continue
    ...
    def latest_run(self) -> dict[str, Any]:
        ...
        result = {**runs[0], "path": run.name,
                  "artifact_availability": {name: True for name in manifest["artifacts"]}}
        summary = artifact_path(run, manifest, "summary")
        result["summary"] = self._load_json(summary)
        return result

    def list_run_files(self, run: str) -> list[dict[str, Any]]:
        folder = self.resolve_run(run)
        manifest = load_completed_manifest(folder)
        return sorted(({"artifact": name, "filename": item["path"], "extension": "." + item["format"],
                        "size": item["bytes"], "sha256": item["sha256"]}
                       for name, item in manifest["artifacts"].items()), key=lambda x: x["artifact"])

    def _report_path(self, run: str, filename: str) -> Path:
        folder = self.resolve_run(run)
        manifest = load_completed_manifest(folder)
        matches = [name for name, item in manifest["artifacts"].items() if item["path"] == filename]
        if len(matches) != 1:
    ...
    def _query_parquet(self, run: str, artifact: str, relation: str, sql: str) -> dict[str, Any]:
        folder = self.resolve_run(run); manifest = load_completed_manifest(folder)
        ...
        with duckdb.connect(":memory:") as connection:
            ...
            connection.execute(f"CREATE VIEW {relation} AS SELECT * FROM read_parquet('{escaped}')")
    ...
    def research_aggregate(self, run: str, spec: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"columns": list(frame.columns), "rows": frame.to_dict(orient="records"),
                "row_count": len(frame), "truncated": False}
    ...
    def compare_runs(self, runs: list[str]) -> list[dict[str, Any]]:
        ...
        for name in runs:
            ...
            manifest = load_completed_manifest(folder); manifests.append(manifest)
            summary = self._load_json(artifact_path(folder, manifest, "summary"))
            row = {"run": name, "symbol": manifest["request"]["symbol"], **manifest["hashes"],
                   "code_commit": manifest.get("code_commit"),
                   "source_snapshot": manifest["catalog"]["catalog_snapshot_digest"],
                   "total_trades": summary.get("total_trades"), "win_rate": summary.get("win_rate"),
                   "net_r": summary.get("total_net_r"), "avg_r": summary.get("average_net_r"),
                   "net_pnl": summary.get("net_pnl")}
            result.append(row)
        baseline = manifests[0]
        for row, manifest in zip(result, manifests):
            row["provenance_vs_first"] = {
                "same_code": manifest.get("code_commit") == baseline.get("code_commit") and manifest.get("code_dirty") == baseline.get("code_dirty"),
                "same_sources": manifest["catalog"]["catalog_snapshot_digest"] == baseline["catalog"]["catalog_snapshot_digest"],
                "same_features": manifest["hashes"]["feature_config_hash"] == baseline["hashes"]["feature_config_hash"] and manifest["features"] == baseline["features"],
                "same_strategy": manifest["hashes"]["strategy_hash"] == baseline["hashes"]["strategy_hash"],
                "same_execution": manifest["hashes"]["execution_hash"] == baseline["hashes"]["execution_hash"],
            }
        return result
...
def main() -> None:
    ...
    LOGGER.info("Available tools: list_runs, latest_run, list_run_files, read_report, query_trades, query_signals, query_telemetry, get_run_manifest, research_aggregate, compare_runs")

...
class MainWindow(QMainWindow):
    ...
    def _build_config(self):
        ...
        self.enable_partial_tp=QCheckBox("Enable Partial Take Profit"); self.tp1_r=self._spin(3.0,0.000001); self.tp1_close_pct=self._spin(50.0,0.000001,100.0); self.tp2_r=self._spin(12.0,0.000001); self.tp2_close_pct=self._spin(50.0,0.000001,100.0); self.stop_loss_r=self._spin(10.0,0.000001); self.after_tp1_stop_mode=QComboBox(); self.after_tp1_stop_mode.addItems(["KEEP_ORIGINAL_SL","MOVE_TO_ENTRY","MOVE_TO_R_OFFSET"]); self.after_tp1_stop_offset_r=self._spin(0.0,0.0); self.tp2_exit_mode=QComboBox(); self.tp2_exit_mode.addItems(["FIXED_TP2","TRAILING_AFTER_TP1"])
        ...
        for lab,w in [("",self.enable_partial_tp),("TP1 (R)",self.tp1_r),("TP1 Close %",self.tp1_close_pct),("TP2 (R)",self.tp2_r),("TP2 Close %",self.tp2_close_pct),("Partial Stop Loss (R)",self.stop_loss_r),("After TP1 Stop",self.after_tp1_stop_mode),("After TP1 Stop Offset (R)",self.after_tp1_stop_offset_r),("TP2 Exit Mode",self.tp2_exit_mode),("Stop Loss Multiple",self.sl),("Take Profit Multiple",self.tp),("Entry Mode",self.entry_mode),("Entry Interval",self.entry_interval),("Maximum Active Pairs",self.max_pairs),("Tie Policy",self.tie),("",self.both_timeout),("Maximum Both-Open Time",timeout_row),("",self.both_timeout_help)]: strat.addRow(lab,w)
        ...
        remaining_timeout_help=QLabel("When one leg hits its normal stop loss, the opposite leg remains open and keeps its TP and active stop. If it is still open after this duration, it is closed at market. The timer starts when the first normal SL is hit."); remaining_timeout_help.setWordWrap(True)
        for lab,w in [("",self.remaining_leg_timeout),("Duration",remaining_timeout_row),("",remaining_timeout_help)]: remaining_timeout.addRow(lab,w)

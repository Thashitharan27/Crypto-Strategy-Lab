...
def test_isolated_only_detaches_heavy_attrs_before_statistics(tmp_path,monkeypatch):
    ...
    class FakeEngine:
        def __init__(self,data,config,intrabar,progress_callback=None,progress_interval=50): pass
        def run(self):
    ...
    def checked_summarize(trades,initial_equity):
        assert "skipped_signals" not in trades.attrs
        return {"ending_equity":initial_equity+1.0,"total_return_percentage":0.1,"win_rate":1.0,"profit_factor":1.0,"maximum_drawdown_percentage":0.0}
    monkeypatch.setattr(worker_module,"BacktestEngine",FakeEngine)
    ...
    monkeypatch.setattr(worker_module,"update_latest",lambda *_:None)
    profiles={key:replace(value,enabled=(key=="bull_long")) for key,value in BacktestConfig().strategy_profiles.items()}
    config=replace(BacktestConfig(),enable_strategy_profiles=True,strategy_profile_run_mode="ISOLATED_PROFILES",strategy_profiles=profiles,output_dir=tmp_path,output_run_dir=tmp_path/"isolated_run")
    worker_module.BacktestWorker(config)._run_isolated_only(pd.DataFrame({"timestamp":pd.to_datetime(["2024-01-01"],utc=True)}),None)

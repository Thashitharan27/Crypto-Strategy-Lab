...
def test_research_features_are_reorganized_without_replacing_authoritative_form():
    ...
    try:
        ...
        assert panel.oi_card.status.text() == "AUTO WHEN AVAILABLE"
        assert panel.funding_card.status.text() == "AUTO WHEN AVAILABLE"
        assert panel.basis_card.status.text() == "AUTO WHEN AVAILABLE"
        assert panel.taker_card.status.text() == "AUTO WHEN AVAILABLE"
